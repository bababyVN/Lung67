...
def create_interface():
    ...
    with gr.Blocks(
        css=custom, theme=gr.themes.Soft(primary_hue="teal", secondary_hue="blue")
    ) as interface:
            ...
            with gr.Column():
                with gr.Group(elem_classes="results-container"):
                    output_image = gr.Image(
                        label="Infection Areas (COVID)", visible=False, height=400
                    )

                with gr.Row(equal_height=True):
                    diagnosis_label = gr.Label(
                        label="Diagnosis Conclusion", elem_classes="results-container"
                    )
                    confidence_label = gr.Label(
                        label="Confidence Score", elem_classes="results-container"
                    )
