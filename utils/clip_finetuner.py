...
class CLIPFineTuner(nn.Module):
    ...
    def __init__(self, clip_model, text_prompts, processor, device):
        ...
        # Encode text prompts once (they don't change)
        with torch.no_grad():
...
def train_clip(model, train_dl, val_dl, device, epochs, lr, save_dir, text_prompts, classes=CLASSES):
    ...
    for epoch in range(1, epochs + 1):
        ...
        # Print epoch summary
        print(f"[CLIP] Ep{epoch}: TrainLoss {train_loss:.3f} (Acc {train_acc:.2f}%) | "
...
def finetune(data_root=DATA_ROOT, 
             classes=CLASSES, 
    ...
    # Train the model
    best_acc = train_clip(model, train_loader, val_loader, device, epochs, lr, 
                         save_dir, text_prompts, classes)
